...
void Dijikstra(int start)
{
	...
	while (true)
	{
		...
		for (int there = 0; there < adjacent.size(); there++)
		{
			...
			//	continue; --> 예약 걸 때 discovered[there] = true로 처리되기 때문에 없어지는게 맞음
			

			// 뒤늦게 찾은 Vertex의 가중치가 기존에 찾은 Vertex의 가중치보다 크면 X
	...
	for (int i = 0; i < adjacent.size(); i++)
	{
		...
		cout << i << "의 0으로 부터의 최단거리 : " << best[i] << endl;
	}
}
